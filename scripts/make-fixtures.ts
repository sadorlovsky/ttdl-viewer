...
const GIVEN_OUT = flag("out");
const OUT = GIVEN_OUT ?? "fixtures/downloads";
const BIG = has("big");
const SEED = Number(flag("seed") ?? 1);

/**
 * Written into every generated tree, and required before a given --out is erased.
 *
 * The first thing this script does is delete --out recursively. That flag is a path somebody
 * types, and the tree it could name — a real library of archives — is the one thing around here
 * that cannot be regenerated. The default path belongs to this script and is erased as before; a
 * path that was asked for by name is erased only if this script is the one that made it.
 */
const MARKER = ".ttdl-viewer-fixtures";
...
interface CarouselOptions {
	...
	emptyImageAt?: number;
	/**
	 * Never write the image at this index at all.
	 *
	 * A refused page leaves a hole anywhere in the sequence, not only after the last image that
	 * arrived, and the two look identical once the URLs are packed into an array.
	 */
	skipImageAt?: number;
...
function addCarouselPost(o: CarouselOptions): string {
	...
	if (!o.noState) {
		...
		const downloaded = Array.from({ length: o.images }, (_, i) => i + 1).filter(
			(i) => i !== o.emptyImageAt && i !== o.skipImageAt,
		);
	...
	const complete =
		o.withMarker ??
		(expectedCount !== null && o.images >= expectedCount && !o.emptyImageAt && !o.skipImageAt);
	if (complete) {
...
function buildTestUser(root: string): void {
	...
	// A hole in the middle: images 1, 2, 4, 5 landed and 3 never did. Everything above leaves its
	// gap at the end, where a strip that counts along the sequence still lands on the right image
	// by accident. Here the fourth segment is the fourth image, not the third.
	addCarouselPost({ archive: a, author, timestamp: next(), images: 5, skipImageAt: 3 });
...
if (GIVEN_OUT && existsSync(OUT) && !existsSync(join(OUT, MARKER))) {
	console.error(
		`refusing to erase ${OUT}: it holds no ${MARKER}, so this script did not generate it.\n` +
			"  --out names the directory that gets deleted first. If that is really what you want,\n" +
...
writeFileSync(join(OUT, MARKER), "generated by scripts/make-fixtures.ts\n");
