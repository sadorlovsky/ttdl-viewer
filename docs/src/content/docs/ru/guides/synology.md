...
Container Manager → **Проект** → **Создать**. Дайте имя, укажите путь вроде
`/volume1/docker/ttdl-viewer` и выберите создание `docker-compose.yml`; мастер принимает содержимое
файла в текстовом поле, так что заранее класть на NAS ничего не нужно:
