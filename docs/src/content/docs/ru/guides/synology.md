...
принадлежат конкретной учётной записи DSM, а не читаемы всем. Посмотрите числового владельца по SSH
и скажите compose совпасть с ним:
...
Прочитать эти числа из интерфейса DSM нельзя, так что этот шаг требует SSH. Панель управления →
Терминал и SNMP → **Включить службу SSH**, если она выключена.
